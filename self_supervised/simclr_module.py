...
def load_data(args, transform_train, transform_test):
    if args.dataset == 'cifar10':
        ...
        val_func = IMBALANCECIFAR10
    train_dataset = train_func(root=args.data_dir, imb_type=args.imb_type, imb_factor=args.imb_factor,
                               rand_number=args.seed, train=True, download=True,
                               transform=transform_train)
    val_dataset = val_func(root=args.data_dir, train=False, download=True,
...
class SimCLR(LightningModule):
    ...
    def on_train_epoch_end(self, dataloader_idx, *args, **kwargs):
        if self.what == "epochs":
            self.state["epochs"] += 1
    ...
    def add_model_specific_args(parent_parser):
        ...
        # model params
        parser.add_argument("--arch", default="resnet34", type=str, help="convnet architecture")
        ...
        parser.add_argument("--num_workers", default=5, type=int, help="num of workers per GPU")
        ...
        parser.add_argument("--max_steps", default=20, type=int, help="max steps")
        parser.add_argument("--warmup_epochs", default=10, type=int, help="number of warmup epochs")
        parser.add_argument("--batch_size", default=128, type=int, help="batch size per gpu")

        parser.add_argument("--temperature", default=0.1, type=float, help="temperature parameter in training loss")
        parser.add_argument("--weight_decay", default=1e-6, type=float, help="weight decay")
        parser.add_argument("--learning_rate", default=1.5, type=float, help="base learning rate")
        parser.add_argument("--start_lr", default=0, type=float, help="initial warmup learning rate")
        parser.add_argument("--final_lr", type=float, default=1e-6, help="final learning rate")
        parser.add_argument("--imb_type", type=str, default='exp', help="type of imbalanced data -step (full), "
                                                                                "exp (full) or  binary_step")
        parser.add_argument("--imb_type_val", type=str, default='exp', help="type of imbalanced val data "
                                                                                    "-step (full), exp (full) or  "
                                                                                    "binary_step")
        parser.add_argument('--imb_factor', default=0.5, type=float, help='imbalance factor')
        parser.add_argument('--imb_factor_val', default=0.5, type=float, help='imbalance factor')
        parser.add_argument(
...
def cli_main():
    ...
    #model_cfg = getattr(models, 'ResNet34')
    ...
    if args.dataset == "stl10":
        ...
        print ('444444', dm.dims)
        train_loader, val_loader, num_classes, samples_weight = load_data(args, train_transforms, val_transforms)
